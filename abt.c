  ...
  struct msg message;
{
  ...
  printf("A_output called Message %d\n",j);
  A_application++;
 packet1.seqnum = j % 2;
 packet1.checksum = 0;
 temp = packet1.seqnum;
packet1.checksum = packet1.checksum + packet1.acknum + packet1.seqnum;
  bzero(packet1.payload, sizeof packet1.payload);
  for(int i=0;i<20;i++)
  {
    packet1.checksum = packet1.checksum + message.data[i];
    packet1.payload[i] = message.data[i];
    message1.data[i] = message.data[i];
  }
printf("A_output checksum: %d\n", packet1.checksum);
 printf("A-onput data %s\n", packet1.payload);
  if(timerrunning == 1)
  {
    stoptimer(0);
  }
  ...
  struct pkt packet;
{
  ...
  printf("Entered A_input\n");
  int checksum2 = 0;
  printf("A_input seqnum: %d acknum: %d\n",packet.seqnum,packet.acknum);
  checksum2 = packet.seqnum + packet.acknum;
  for(int h=0;h<20;h++)
  {
    checksum2 = checksum2 + packet.payload[h];  
  }
  //printf("A_input data %s\n", data_rcv);
 printf("A_input checksum2: %d checksum: %d\n", checksum2, packet.checksum);
if(checksum2 == packet.checksum)
{
  ...
  struct pkt packet;
{
  ...
  printf("B_input seqnum: %d acknum: %d\n",packet.seqnum,packet.acknum);
   for(int i=0;i<20;i++)
  {
    checksum1 = checksum1 + (int)packet.payload[i];
  }
  printf("B_input checksum1: %d\n",checksum1);

  if(checksum1 == packet.checksum)
  {
    printf("Packet not corrupt\n");
    if(packet.seqnum != packet2.seqnum)
    {
      tolayer5(1, packet.payload);
      B_application++;
    }
    bzero(packet2.payload, sizeof packet2.payload);
    packet2.checksum = 0;
    packet2.seqnum = packet.seqnum;
    packet2.acknum = packet.seqnum;
    packet2.checksum = (int)packet2.acknum + (int)packet2.seqnum;
    for(int t=0;t<20;t++)
    {
      packet2.checksum = packet2.checksum + (int)packet2.payload[t];
    }
    printf("B_input checksum2: %d\n",packet2.checksum);
    tolayer3(1,packet2);
  }
  else
  {
