...
void B_input(packet)
  struct pkt packet;
{
  printf("B_output has received message: %d\n", packet.seqnum);
  printf("EXPECTED SEQ NUMBER at B: %d\n", expectedseqnum);
  B_transport++;
  int checksum2 = 0;
  checksum2 = packet.seqnum + packet.acknum;
  for(int i=0;i<20;i++)
  {
    checksum2 = checksum2 + packet.payload[i];
  }
 // if(packet.acknum == -1)
  //  {
   //   tolayer3(1,Ack);
  ...
  if(checksum2 == packet.checksum)
  {
    ...
    if(packet.seqnum == expectedseqnum)
    {
        ...
        for(int i=0;i<20;i++)
        {
          Ack.checksum = Ack.checksum + Ack.payload[i];
        }
        tolayer3(1,Ack);
        expectedseqnum++;
    ...
    {
      printf("Unexpected sequence number arrived\n");
      Ack1.seqnum = expectedseqnum-1;
        Ack1.acknum = expectedseqnum-1;
        bzero(Ack1.payload, sizeof Ack1.payload);
        Ack1.checksum = Ack1.seqnum + Ack1.acknum;
        for(int i=0;i<20;i++)
        {
          Ack1.checksum = Ack1.checksum + Ack1.payload[i];
        }
        tolayer3(1,Ack1);
    }
  }
